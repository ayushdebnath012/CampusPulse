...
- Expired sessions and codes are dropped on every load, and each inbox keeps its newest fifteen notifications for thirty days. Every write moves the whole document to the database and back, and a term of "attendance is open" alerts to a class of 310 had grown it by megabytes.
...
- A refused write no longer costs the room. A check-in refused for a wrong code or a far location throws from inside its mutator, and the batching updater used to discard the whole batch and replay it one writer at a time — one round trip of the document became sixty, serialised — and then drop the read cache so the next reader reloaded it all again. Now the refused writer is rejected on its own and the rest of the batch reruns once; the cache is kept, because a rolled-back transaction changed nothing.

What the production logs showed after the first term's feedback, for whoever looks next: the t3.micro never went above 20% CPU or spent a CPU credit, and RDS was idle. Every request in a class hour came from a mobile carrier — not one from the campus network — so students were on mobile data throughout, while the app was demanding Wi‑Fi before it would mark them. And every refused check-in was a stale beacon token, none a location failure. The bottleneck was the code, not the instance.
