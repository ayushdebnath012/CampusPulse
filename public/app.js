...
// Reset by email is offered unless the server reports that it cannot send
// one. Starting hidden meant a slow or unreachable health probe left locked-out
// students with no way back in.
let emailDeliveryAvailable = true;
...
async function refreshEmailDeliveryState() {
  ...
  try {
    ...
    emailDeliveryAvailable =
      Boolean(health.emailDelivery) && health.emailDelivery !== "disabled";
...
// Toggled in place rather than redrawn, so an email or password already typed
// into the sign-in form survives the health probe answering.
function syncForgotPasswordLink() {
  authRoot?.querySelectorAll("[data-forgot-link]").forEach(link => {
    link.hidden = !emailDeliveryAvailable;
  });
  authRoot?.querySelectorAll("[data-reset-unavailable]").forEach(note => {
    note.hidden = emailDeliveryAvailable;
  });
}
...
// Returning users are the common case, so the door opens on sign-in; a new
// student switches to "Create account" themselves.
let authMode = "login";
      ...
      <section class="auth-panel">
        <div class="auth-card">
          ...
          <form id="loginForm" class="login-form">
            ...
            <div class="label-row"><label for="loginPassword">Password</label><button type="button" class="text-btn" data-auth-mode="forgot" data-forgot-link ${emailDeliveryAvailable ? "" : "hidden"}>Forgot password?</button></div>
          ...
          <div class="auth-demo-note"><span>Secure password sign-in</span><p>Use the email, password, and account role selected during sign-up.<span data-reset-unavailable ${emailDeliveryAvailable ? "hidden" : ""}> Password reset by email is switched off, so ask your professor if you are locked out.</span></p></div>`}`}
